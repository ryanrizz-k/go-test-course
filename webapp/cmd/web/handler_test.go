...
func Test_app_Login(t *testing.T) {
	var tests = []struct {
	...
	}{
		{
			...
			expedtedStatusCode: http.StatusSeeOther,
	...
	for _, e := range tests {
		...
		if rr.Code != e.expedtedStatusCode {
			t.Errorf("%s: returned wrong status code; expected %d, but got %d", e.name, e.expedtedStatusCode, rr.Code)
		}
...
func simulatePNGUpload(fileToUpload string, writer *multipart.Writer, t *testing.T, wg *sync.WaitGroup) {
	...
	// write the png to out io.Writer
