...
func Test_app_authRequired(t *testing.T) {
	...
	var tests = []struct {
		name            string
		token           string
		expecAuthorized bool
		setHeader       bool
	}{
		{name: "valid token", token: fmt.Sprintf("Bearer %s", tokens.Token), expecAuthorized: true, setHeader: true},
		{name: "no token", token: "", expecAuthorized: false, setHeader: false},
		{name: "invalid token", token: fmt.Sprintf("Bearer %s", expiredToken), expecAuthorized: false, setHeader: true},
	}
	...
	for _, e := range tests {
		...
		if e.expecAuthorized && rr.Code == http.StatusUnauthorized {
