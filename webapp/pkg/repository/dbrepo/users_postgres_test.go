...
func TestPostgresDBRepoGetUserByEmail(t *testing.T) {
	...
	if user.ID != 2 {
		t.Errorf("wrong id returned by GetUser; expected jack@smith.com but got %d", user.ID)
	}
}
...
func TestPostgresDBRepoUpdateUser(t *testing.T) {
	...
	if user.FirstName != "Jane" || user.Email != "jane@smith.com" {
		t.Errorf("expected updated record to have to have first name Jane and email jane@smith.com, but got %s %s", user.FirstName, user.Email)
	}
}
...
func TestPostgresDBRepoResetPassword(t *testing.T) {
	...
	if err != nil {
		t.Error("erorr resetting user's password", err)
	}
...
func TestPostgresDBRepoInserUserImage(t *testing.T) {
